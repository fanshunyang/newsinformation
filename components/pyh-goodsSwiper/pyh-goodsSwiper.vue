<template>
	<view class="goodsSwiperBox" v-show="detail">
		<view class='gswTitle'>
			...
			<view class="gswTR" @tap='tapFun' data-type="more" v-if="config.more">更多<image src='/static/pyh-goodsSwiper/goIcon.png' style="margin-right: -10rpx;"></image><image src='/static/pyh-goodsSwiper/goIcon.png'></image></view>
		</view>
		<swiper class="gswSw" v-if="detail.list&&detail.list.length>0" :display-multiple-items="config.multiple||3" :autoplay="config.autoplay" :interval="config.interval||5000" :duration="config.duration||500" :circular="config.circular||false" :next-margin="config.hideMargin?'0':'100rpx'" :current="config.current||0" @change="swiperChange" :disable-touch="disable">
...
<style>
	...
	.gswSw{height: 250rpx;padding-bottom: 20rpx;}
	.gswGoods{width: 100%;padding-left: 10rpx;font-weight: bold;}
	.gswGoods image{width: 100%;height: 176rpx;border-radius: 10rpx;display: block;}
	.gswGoods .goodsTxt{text-overflow: ellipsis;overflow: hidden;white-space: nowrap;font-size: 24rpx;line-height: 33rpx;text-align: center;}
