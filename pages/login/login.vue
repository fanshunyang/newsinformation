	...
	export default {
		...
		methods: {
			...
			QQ () {
				...
				// 	service:'oauth',
				// 	success(res) {
				// 		const numbers = res.provider.indexOf('qq')
				// 		if (numbers===0) {
				// 			uni.login({
				// 				provider:'qq',
				// 				success(res) {
				...
				// 				  	  success: async (res)=> {
				// 						  console.log(res) 
				// 						  const {openId,nickName,figureurl_qq} = res.userInfo
				// 						  console.log( openId, nickName , figureurl_qq)
				// 						  //发送请求
				// 						// const data = await this.$http.post('/api/userLogin',{
				// 						// nickname:nickName,
				// 						// avatarurl:avatarUrl,
			...
			phoneNumbers () {
			 uni.showModal({
			 	title:'该登录请确定自己是否有手机卡并且确保自己的数据网络是否已经开启',
				success:(res)=> {
					if (res.confirm) {
						// let _this = this
						uni.preLogin({
